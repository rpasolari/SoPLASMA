/*---------------------------------------------------------------------------*\
  File: tabulatedMobility2D.C
  Part of: foamPlasmaToolkit
  Developed using the OpenFOAM framework and linked against OpenFOAM libraries.

  Description:
    Implementation of Foam::tabulatedMobility2D.
...
defineTypeNameAndDebug(tabulatedMobility2D, 0);
...
tabulatedMobility2D::tabulatedMobility2D
(
    ...
    lookupVariables_(dict.get<wordList>("lookupVariables"))
{
    ...
    {
        FatalIOErrorInFunction(dict)
            << "tabulatedMobility2D requires exactly 2 lookupVariables, "
...
void tabulatedMobility2D::correct(volScalarField& mu) const
{
    ...
    forAll(mu, cellI)
    {
        mu[cellI] = table_(fieldX[cellI], fieldY[cellI]); 
    }
